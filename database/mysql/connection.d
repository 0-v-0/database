...
/++ Database socket type used by the MySQL connection implementation. +/
alias Socket = DBSocket!MySQLConnectionException;

/++ Default client capability flags used when establishing a new MySQL connection. +/
...
/++ Runtime status counters returned by MySQL status packets. +/
struct Status {
	/++ Number of rows affected by the last server command. +/
	ulong affected;
	/++ Number of rows matched by the last UPDATE/SELECT operation. +/
	ulong matched;
	/++ Number of rows changed by the last data-modifying command. +/
	ulong changed;
	/++ Last auto-increment identifier generated by INSERT. +/
	ulong lastInsertId;
	/++ Current server status bitfield for the connection. +/
	ushort flags;
	/++ Last server error code (or zero when no error). +/
	ushort error;
	/++ Number of server warnings from the last operation. +/
	ushort warnings;
}
...
class Connection {
	/++ Active MySQL connection wrapper.

Handles low-level protocol messages, prepared statements and query execution.
+/
	...
	/++ Select the default database for this connection.

Issues `COM_INIT_DB` and updates internal schema state.
+/
	void use(in char[] db) {
	...
	/++ Send `COM_PING` and validate the server status response. +/
	void ping() {
		send(Commands.COM_PING);
		eatStatus(retrieve());
	}

	/++ Send a server refresh command. +/
	void refresh() {
		send(Commands.COM_REFRESH);
		eatStatus(retrieve());
	}

	/++ Reset the current connection state on the server. +/
	void reset() {
		send(Commands.COM_RESET_CONNECTION);
		eatStatus(retrieve());
	}

	/++ Fetch connection statistics from the server. +/
	...
	/++ Return the current default schema for this connection. +/
	const(char)[] schema() const
		=> schema_;

	/++ Prepare a SQL statement and return a server statement handle. +/
	...
	/++ Alias of `query`; retained for API compatibility. +/
	alias executeNoPrepare = query;

	/++ Execute an SQL statement and optionally dispatch each row to a handler. +/
	...
	/++ Execute a prepared or ad-hoc SQL statement using positional args.

Reuses server-side prepared statements for repeated SQL text.
+/
	...
	/++ Set a session variable value for this connection. +/
	void set(T)(const(char)[] variable, T value) {
		query("set session ?=?", MySQLFragment(variable), value);
	}

	/++ Query `show session variables like ?` and return the first match. +/
	...
	/++ Roll back the current transaction if one is active. +/
	void rollback() {
		if (!connected)
			return;
	...
	/++ Execute a prepared statement with parameter arguments.

If a trailing callback is provided, rows are passed to it; otherwise the
result set is drained and discarded.
+/
	...
	/++ Last insert identifier from the most recent insert operation. +/
	@property ulong lastInsertId() const => status_.lastInsertId;

	/++ Number of affected rows reported by the last command. +/
	@property ulong affected() const => cast(size_t)status_.affected;

	/++ Number of matched rows reported by the last command. +/
	@property ulong matched() const => cast(size_t)status_.matched;

	/++ Number of changed rows reported by the last command. +/
	@property ulong changed() const => cast(size_t)status_.changed;

	/++ Count of warnings reported by the last server status packet. +/
	@property size_t warnings() const => status_.warnings;

	/++ Error code from the last server status packet. +/
	@property size_t error() const => status_.error;

	/++ Last textual status message (for protocol diagnostics). +/
	@property const(char)[] status() const => info_;

	/++ Connection health check based on socket liveness. +/
	@property bool connected() const => socket && socket.isAlive;

	/++ Close the socket and reset connection object state. +/
	void close() nothrow {
	...
	/++ Prepare the connection for reuse in the pool.

Re-establishes connectivity and restores schema when needed.
+/
	void reuse() {
		ensureConnected();
	...
	/++ Enable or disable query tracing output in exceptions/logging. +/
	@property void trace(bool enable) {
		trace_ = enable;
	}

	/++ Whether query tracing is enabled for this connection. +/
